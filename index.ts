...
class Crypto{
    ...
    CanSelected(val:boolean):void{
        this.canBeSelected = val
    }

    DisplayToggleSwitch():string{
        let checked = `${this.isSelected? 'checked' : ''}`
        if (this.canBeSelected) return `<label><input type="checkbox" class="switch" ${checked}><span class="slider"></span></label>`
        return ''
    }
...
function DisplayCurrencies(theList:Crypto[]){
    if (!$(".ctnr").hasClass("card-deck")) $(".ctnr").addClass("card-deck")
    if (!$(".ctnr").hasClass("row")) $(".ctnr").addClass("row")
    $(".ctnr").empty()
...
function OnMoreInfo(e:JQuery.ClickEvent){
...
function OnToggleSwitchChanged(e:JQuery.ChangeEvent){
...
function RedrawCoin(coinName:string, isChecked:boolean):void{
...
function AskForExchangeRates():void{
